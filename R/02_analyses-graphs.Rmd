...
data_method <- data_benthos %>% 
  ...
  count(name = "Abs") %>% 
  ungroup() %>% 
  pivot_wider(., names_from = Method, values_from = Abs, values_fill = 0) %>% 
  pivot_longer(., cols = c("Photo-quadrat", "PIT", "LIT", "Visual census", "Other methods", "Unknown"), names_to = "Method") %>% 
  rename(., Abs = value) %>% 
  group_by(SOVEREIGN1) %>% 
