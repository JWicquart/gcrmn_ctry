...
data_method <- data_benthos %>% 
  ...
  mutate(Rel = (Abs/sum(Abs))*100,
        ...
        Method = factor(Method, levels = rev(c("Photo-quadrat", "Visual census", "LIT", "PIT", "Other methods", "Unknown")))) %>% 
  ungroup() %>% 
  arrange(SOVEREIGN1)
...
for (i in unique(data_method$SOVEREIGN1)) {
  ...
  plot_i <- data_method %>% 
    ...
    scale_fill_manual(breaks = c("Photo-quadrat", "Visual census", "LIT", "PIT", "Other methods", "Unknown"),
...
data_method <- data_benthos %>% 
  ...
  mutate(Rel = (Abs/sum(Abs))*100,
         Method = as.factor(Method),
         Method = factor(Method, levels = c("Photo-quadrat", "Visual census", "LIT", "PIT", "Other methods", "Unknown"))) %>% 
  ungroup() %>% 
  arrange(SOVEREIGN1)
...
for (i in unique(data_method$SOVEREIGN1)) {
  ...
  plot_i <- ggplot() +
    ...
    scale_fill_manual(name = "Method",
                      breaks = c("Photo-quadrat", "Visual census", "LIT", "PIT", "Other methods", "Unknown"),
                      values = c("#16a085", "#00b16a", "#00b5cc", "#3498db", "#19b5fe", "#6c7a89"),
                      drop = FALSE) +
