...
#library(rfishbase) # Access to the fishbase database
...
### 
#temp <- data_benthos %>% 
#  filter(SOVEREIGN1 %in% "United States") %>% 
#  distinct(Country, Area, Archipelago, .keep_all = T)
```
...
monit_effort <- left_join(nb_sites, nb_obs) 

write_csv2(monit_effort, "../figs/monitoring-effort-by-country.csv")
...
data_method %>% 
  ...
  select("SOVEREIGN1", "Time_range", "LIT", "Other methods", "Photo-quadrat",
         "PIT", "Unknown", "Visual census") %>% 
  group_by(SOVEREIGN1) %>% 
  do(write_csv2(., paste0("../figs/03_table-methods-used-by-time-range_", tolower(unique(.$SOVEREIGN1)), ".csv")))

```

# Taxonomic precision general
...
# Taxonomic precision hard corals

```{r}

# 1. Modify the data ----

taxo_coral <- data_benthos %>% 
  filter(Category == "Hard living coral") %>% 
  mutate(Group = ifelse(is.na(Group), "No", "Yes")) %>% 
...
for (i in unique(taxo_coral$SOVEREIGN1)) {

  precision_taxo_i <- taxo_coral %>% 
    filter(SOVEREIGN1 == i)
  
  plot_i <- ggplot(data = precision_taxo_i, aes(x = SOVEREIGN1, y = row_rel, fill = Group)) +
  geom_bar(stat = "identity") +
  coord_flip() + 
  scale_fill_manual(breaks = c("Yes", "No"),
                    values = c("#03a678", "#d64541"),
                    name = "Shape",
                    drop = FALSE) +
  ...
  ggsave(plot_i, 
         filename = paste0("../figs/02_graph-rows-precision-coral_", tolower(as.character(i)), ".png"),
         width = 6,
         height = 2)
}
...
for (i in unique(taxo_algae$SOVEREIGN1)) {
  ...
  precision_taxo_i <- taxo_algae %>% 
    filter(SOVEREIGN1 == i)
  
  plot_i <- ggplot(data = precision_taxo_i, aes(x = SOVEREIGN1, y = row_rel, fill = Group)) +
  geom_bar(stat = "identity") +
  coord_flip() + 
  scale_fill_manual(breaks = c("Coralline algae", "Macroalgae", "Turf algae", "Unknown"),
  ...
  ggsave(plot_i, 
         filename = paste0("../figs/02_graph-rows-precision-algae_", tolower(as.character(i)), ".png"),
         width = 6,
         height = 2)
}
...
for (i in unique(taxo_othfau$SOVEREIGN1)) {
  ...
  precision_taxo_i <- taxo_othfau %>% 
    filter(SOVEREIGN1 == i)
  
  plot_i <- ggplot(data = precision_taxo_i, aes(x = SOVEREIGN1, y = row_rel, fill = Group)) +
  geom_bar(stat = "identity") +
  coord_flip() + 
  scale_fill_manual(breaks = c("Alcyonacea", "Other groups", "Porifera", "Tunicata", "Unknown"),
  ...
  ggsave(plot_i, 
         filename = paste0("../figs/02_graph-rows-precision-other-fauna_", tolower(as.character(i)), ".png"),
         width = 6,
         height = 2)
}
