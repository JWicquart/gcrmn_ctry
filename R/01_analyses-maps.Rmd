...
#ggplot() + geom_sf(data = coral_distribution)
```
# 1.2. MPAs
...
land_atolls_trans <- land_atolls %>% 
  st_transform(., crs = crs_global)
# 2. Select the 3 countries ----
top_30 <- data_boundary %>% 
  select(SOVEREIGN1) %>% 
  distinct(.) 
# 3. Create all the plots ----
...
for (i in list_classic) {
  ...
  geom_sf(data = land_minor, fill = col_fill_map, col = col_color_map) +
  geom_sf(data = coral_distribution,
          col = "azure3", size = rel(0.2), alpha = 0.5,
          show.legend = FALSE) +
  #geom_sf(data = data_eez_i, color = col_eez_b, fill = NULL) +
  geom_sf(data = land_atolls, fill = col_fill_map, col = col_color_map) +
  geom_sf(data = land_boundary, fill = col_fill_map, col = col_color_map) +
  # Adding the location sites
...
# 3.2 Overlap maps
# 3.2.1 Projection overlaps

```{r}
# 1. Load data ----
# 1.2 Land major --
land_major_ovlp <- readOGR("../data/01_maps/ne_10m_land/ne_10m_land.shp", "ne_10m_land") 
land_major_ovlp <- fortify(land_major_ovlp)
land_major_ovlp <- land_major_ovlp %>% 
  mutate(long = long + 360) %>% 
  bind_rows(land_major_ovlp, .)
# 1.3 Land minor --
land_minor_ovlp <- readOGR("../data/01_maps/ne_10m_minor_islands/ne_10m_minor_islands.shp", "ne_10m_minor_islands") 
land_minor_ovlp <- fortify(land_minor_ovlp)
land_minor_ovlp <- land_minor_ovlp %>% 
  mutate(long = long + 360) %>% 
  bind_rows(land_minor_ovlp, .)
# 1.4 Land atolls --
land_atolls_ovlp <- readOGR("../data/01_maps/ne_10m_reefs/ne_10m_reefs.shp", "ne_10m_reefs")
land_atolls_ovlp <- fortify(land_atolls_ovlp)
land_atolls_ovlp <- land_atolls_ovlp %>% 
  mutate(long = long + 360) %>% 
  bind_rows(land_atolls_ovlp, .)
# 1.5 Coral reefs
...
data_benthos_range_fiji <- data_benthos_range %>% 
  ...
  st_drop_geometry() %>% 
  mutate(long = ifelse(long < 0, long + 360, long),
         #plot the longest time series on top of the others
         intervall_class = factor(interval_class, levels = c("1", "2-5", "6-10", "11-20", ">20"))) %>% 
  arrange(interval_class)

# 2. Make the plot ----
ggplot() +
  geom_polygon(data = data_eez_fiji, aes(x = long, y = lat, group = group), fill = col_eez, alpha = 0.8) +
  geom_point(data = coral_distribution_ovlp, aes(x= long, y = lat), 
             color = "azure3", size = rel(0.2), alpha = 0.5,
             show.legend = FALSE) +
  geom_polygon(data = land_major_ovlp, aes(x = long, y = lat, group = group), fill = col_fill_map, col = col_color_map) +
  geom_polygon(data = land_minor_ovlp, aes(x = long, y = lat, group = group), fill = col_fill_map, col = col_color_map) +
  geom_polygon(data = land_atolls_ovlp, aes(x = long, y = lat, group = group), fill = col_fill_map, col = col_color_map) +
  
  geom_point(data = data_benthos_range_fiji, shape = 21, col = "black", size = 2.5, aes(x = long, y = lat,  fill = interval_class)) + 
  guides(colour = guide_legend(override.aes = list(size = 4))) +
  scale_color_manual(values = palette_trange, name = "Time range (years)", drop = FALSE) +
  scale_x_continuous(breaks = c(170, 175, 180, 185), labels = c("170°E","175°E", "180°", "175°W"), limits = c(170, 185)) +
...
# 1. Load data ----
# 1.1 EEZ data --
data_eez_kiri <- readOGR("../data/01_maps/World_EEZ_v11_20191118/eez_v11.shp", "eez_v11")
data_eez_kiri <- subset(data_eez_kiri, SOVEREIGN1 == "Kiribati")
data_eez_kiri <- fortify(data_eez_kiri)
data_eez_kiri <- data_eez_kiri %>% 
  mutate(long = long + 360) %>% 
  bind_rows(data_eez_kiri, .)
# 1.2 Data benthos range --
data_benthos_range_kiri <- data_benthos_range %>% 
...
# 2. Make the plot ----
ggplot() +
  geom_polygon(data = data_eez_kiri, aes(x = long, y = lat, group = group), fill = col_eez, alpha = 0.8) +
  geom_point(data = coral_distribution_ovlp, aes(x= long, y = lat), 
             color = "azure3", size = rel(0.2), alpha = 0.5,
             show.legend = FALSE) +
  geom_polygon(data = land_major_ovlp, aes(x = long, y = lat, group = group), fill = col_fill_map, col = col_color_map) +
  ...
  geom_point(data = data_benthos_range_kiri, shape = 21, col = "black", size = 3, aes(x = long, y = lat, fill = interval_class)) +
  guides(colour = guide_legend(override.aes = list(size = 4))) +
  scale_color_manual(values = palette_trange, name = "Time range (years)", drop = FALSE) +
  scale_x_continuous(breaks = c(170, 180, 190, 200, 210), 
                     labels = c("170°E","180°", "170°W", "160°W", "150°W"),
                     limits = c(165, 215)) +
  scale_y_continuous(breaks = c(-15, -10, - 5, 0, 5, 10), 
                     labels = c("15°S", "10°S", "5°S", "0°", "5°N", "10°N"), 
                     limits = c(c(-15, 10))) +
  coord_quickmap() +
  guides(fill = guide_legend(override.aes = list(size = 4))) +
  scale_fill_manual(values = palette_trange, name = "Time range (years)", drop = FALSE) +
  ...
  #annotation_scale(location = "bl", plot_unit = "km") 
# 3. Export the map ----
ggsave(filename = "../figs/01_individual-map-by-country_kiribati.png")
# 4. Remove useless objects ----
rm(data_eez_kiri, land_major_kiri, land_minor_kiri, land_atolls_kiri, data_benthos_range_kiri)
# Another solution
# https://stackoverflow.com/questions/58662438/sf-ggplot-trying-to-map-fiji-islands
```
...
```{r}
#1. Select the data for the French overseas territories
...
plot_i <- ggplot() +
  ...
  geom_sf(data = land_boundary, fill = col_fill_map, col = col_color_map) +
  # Adding the location sites
  geom_sf(data = data_benthos_fr, shape = 21, col = "black", aes(fill = interval_class), size = 3) +
  # Define boundaries
  coord_sf(xlim = c(data_boundary_i$lon_min, data_boundary_i$lon_max), 
           ylim = c(data_boundary_i$lat_min, data_boundary_i$lat_max), expand = FALSE) +
  # Aesthetics
...
# 2.6. Mascarene and Eparses Islands
data_boundary_i <- data_boundary %>% 
...
temp <- data_benthos_range %>% 
  ...
  st_drop_geometry() %>% 
  distinct(Area, Country, Location, .keep_all = T)
data_eez_us <- data_eez %>% 
    filter(SOVEREIGN1 == "United States")
  
data_benthos_us <- data_benthos_range %>% 
  filter(SOVEREIGN1 == "United States") 
#2.6. Global map USA
plot_i <- ggplot() +
  # Adding the EEZ
  geom_sf(data = data_eez_us, fill = col_eez, color = col_eez_b, alpha = 0.8) +
  # Adding territories
  geom_sf(data = land_major, fill = col_fill_map, col = col_color_map) +
  geom_sf(data = land_minor, fill = col_fill_map, col = col_color_map) +
  geom_sf(data = coral_distribution,
  ...
  geom_sf(data = land_boundary, fill = col_fill_map, col = col_color_map) +
  # Adding the location sites
  geom_sf(data = data_benthos_us, shape = 21, col = "black", aes(fill = interval_class), size = 3) +
  # Define boundaries
  coord_sf(ylim = c(-50,50), expand = FALSE) +
  # Aesthetics
  guides(fill = guide_legend(override.aes = list(size = 4))) +
  scale_fill_manual(values = palette_trange, name = "Time range (years)", drop = FALSE) +
  theme(axis.title = element_blank())
ggsave(plot_i, filename = paste0("../figs/01_individual-map-for-usa_global.png"))
# 2. Make the individual maps
# 2.1. French Antillas
data_boundary_i <- data_boundary %>% 
    filter(sub_country == "caribbean")
...
plot_i <- ggplot() +
  ...
  geom_sf(data = land_boundary, fill = col_fill_map, col = col_color_map) +
  # Adding the location sites
  geom_sf(data = data_benthos_us, aes(col =  interval_class)) +
  # Define boundaries
  coord_sf(xlim = c(data_boundary_i$lon_min, data_boundary_i$lon_max), 
           ylim = c(data_boundary_i$lat_min, data_boundary_i$lat_max), expand = FALSE) +
  # Aesthetics
...
ggsave(plot_i, filename = paste0("../figs/01_individual-map-for-usa_caribbean.png"))
#rm(data_eez_us, data_benthos_us)
```
