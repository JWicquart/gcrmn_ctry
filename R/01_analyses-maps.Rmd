...
# 4.2 Data benthos - Summarised with time-range (top 30) --
...
data_eez <- read_sf("../data/01_maps/World_EEZ_v11_20191118/eez_v11.shp") %>% 
  filter(SOVEREIGN1 %in% unique(data_benthos$SOVEREIGN1)) # Extract top 30 countries

# 4.4 Background maps --

load(file = "./../data/01_maps/00_background_map.RData")

# 4.5 Background maps country boundaries --
...
# 1.1 Define the CRS of global maps (Mollweide) --
...
for (i in as.character(unique(data_eez$SOVEREIGN1))) {
...
rm(world_map_background, plot_i, data_eez_i, base_ggplot, crs_global)
...
for (i in data_bbox_classic) {
  ...
  ggsave(plot_i, filename = paste0("../figs/01_individual-map-by-country_", str_replace_all(tolower(i), " ", "-"), ".png"))
...
data_eez_sovereign <- data_eez %>% 
  filter(TERRITORY1 %in% c("French Polynesia", "Mayotte", "Ile Tromelin", "New Caledonia", 
                           ...
                           "Clipperton Island", "Guadeloupe", "Saint-Barthélemy",
...
plot_baeu <- ggplot() +
  ...
  # Benthos
  geom_sf(data = data_benthos_range_i, shape = 21, col = "black", aes(fill = interval_class), size = 3, show.legend = FALSE) +
  coord_sf(xlim = c(37, 42), ylim = c(-26, -19)) +
  scale_fill_manual(values = palette_trange, name = "Time range (years)", drop = FALSE) +
  theme(panel.background = element_blank(),
        panel.border = element_blank(),
        axis.title = element_blank(),
...
plot_fr <- ggarrange(ggarrange(ggplot_map_eez(territory_name = "Collectivity of Saint Martin", title = TRUE),
                               ...
                               ggplot_map_eez(territory_name = "Martinique", title = TRUE),
                               ggplot_map_eez(territory_name = "Guadeloupe", title = TRUE), ncol = 4),
                     ggplot_map_global(), 
                     ggarrange(plot_wafu,
                               ggplot_map_eez(territory_name = "French Polynesia", title = TRUE),
                               ggplot_map_eez(territory_name = "Clipperton Island", title = TRUE),
                               ggplot_map_eez(territory_name = "New Caledonia", title = TRUE), ncol = 4), 
                     ggarrange(ggplot_map_eez(territory_name = "Mayotte", title = TRUE),
                               ggplot_map_eez(territory_name = "Juan de Nova Island", title = TRUE),
                               ggplot_map_eez(territory_name = "Réunion", title = TRUE),
                               plot_baeu,
                               ggplot_map_eez(territory_name = "Ile Tromelin", title = TRUE), ncol = 5),
                     nrow = 4, heights = c(1, 2, 1, 1))

# 5. Save the plot ----

ggsave(plot_fr, file = "../figs/01_individual-map-by-country_france.png", width = 10, height = 12)
...
ggsave(plot_us, file = "../figs/01_individual-map-by-country_united-states.png", width = 10, height = 12)
...
plot_uk <- ggarrange(ggarrange(ggplot_map_eez(territory_name = "Turks and Caicos Islands", title = TRUE),
                    ...
                    ggplot_map_eez(territory_name = "Bermuda", title = TRUE), ncol = 3),
          ggplot_map_global(), 
...
ggsave(plot_uk, file = "../figs/01_individual-map-by-country_united-kingdom.png", width = 10, height = 8)
