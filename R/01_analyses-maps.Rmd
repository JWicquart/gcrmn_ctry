...
# Extract MPAs for 6 selected countries ----
data_mpa_top6 <- data_mpa %>% 
  filter(sovereign %in% c("FJI", "FRA", "IDN", "BLZ", "EGY", "MDV", "CUB")) %>% 
  filter(is_mpa == 1) %>% 
  group_by(sovereign) %>% 
...
units(data_mpa_top6$surface_mpa) <- "km^2"

## Extract all the MPAs that contain coral reefs for xXXX/ the top 6 ----
...
coral_distribution_fiji <- coral_distribution %>% 
  mutate(long = ifelse(long < 0, long + 360, long)) 

# 1.6 Data benthos range --
...
ggplot() +
  ...
  geom_point(data = coral_distribution_fiji, aes(x= long, y = lat), 
