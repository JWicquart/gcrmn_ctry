...
data_mpa_top30 <- data_mpa %>% 
  filter(sovereign %in% c("AUS", "IDN", "PHL", "FRA", "PNG", "USA", "FJI", "SLB", "SAU", "MDV",
                          ...
                          "MUS", "MOZ", "SDN", "ERI", "JPN", "SYC", "BLZ", "GBR", "VUT", "TON")) %>% 
  filter(is_mpa == 1) %>% 
  group_by(sovereign) %>% 
...
## Joining with the EEZ ----
# Calculate the % of the EEZ covered with MPAs

data_eez_6 <- data_eez %>% 
  filter(SOVEREIGN1 %in% c("Belize", "Egypt", "Fiji", "France", "Indonesia", "Maldives")) %>% 
  mutate(surf_eez = st_area(geometry)) %>% 
  #st_drop_geometry() %>% 
  group_by(SOVEREIGN1) %>% 
  summarise(surface_eez = sum(surf_eez))
  
test <- data_mpa %>% 
  filter(sovereign %in% c("FJI", "FRA", "IDN", "BLZ", "EGY", "MDV")) %>% 
  st_join(., data_eez_6)  %>% 
  st_drop_geometry() 
  
  
  group_by(sovereign) %>% 
  mutate(mpa_vs_eez <- surf_mpa/surf_eez * 100)
...
results <- data_eez_trans %>%
  filter(SOVEREIGN1 %in% c("Belize", "Fiji", "France", "Egypt", "Maldives", "Indonesia", "Cuba")) %>% 
  group_by(SOVEREIGN1) %>%
  do(plot = ggplot() +
